...
data Hashtable a = Hashtable
    { hashtableKeys :: {-# UNPACK #-} !(IOArray Int BS.ByteString)
    , hashtableValues :: {-# UNPACK #-} !(IOArray Int a)
    }
...
freeze :: forall a. Hashtable a -> IO (FrozenHashtable a)
freeze ht = do
...
insertWith :: forall a. (a -> a -> a) -> BS.ByteString -> a -> Hashtable a -> IO Bool
